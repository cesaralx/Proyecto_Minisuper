...
//                txtNroCompT.setText(String.valueOf(rs.getString(1)));
                for (int i = 0; i < user.cTran; i++) {
                 ...
                 txtNroCompT.setText(String.valueOf(user.aTran[i]));
                 }
    ...
    public void AgregarTransaccion() {
        ...
        try {
            Producto p = datosProducto.buscarProducto(Integer.parseInt(txtIdProdT.getText()));
            Compra comp = datosCompra.buscarCompra(Integer.parseInt(txtNroCompT.getText()));
            t.setProducto(p);
            t.setCompra(comp);
            datosTransaccion.insertarTransaccion(t);
        } catch (Exception e) {
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        ...
        try {
            Producto p = datosProducto.buscarProductoNombre(String.valueOf(jtTransacProd.getModel().getValueAt(elim_fila, 1)));
            int idp = p.getIdProducto();
            datosTransaccion.eliminarTransaccion(idcom, idp);
